...
"""Cell 11 – Reserved Unseen Evaluation"""
...
# Path to reserved unseen images
unseen_folder = "/content/drive/MyDrive/reserved_unseen"
onnx_model_path = "/content/drive/MyDrive/Capstone_Project_FAW_Dataset_v4.1/faw_cnn_model.onnx"

# Initialize ONNX Runtime session
ort_session = ort.InferenceSession(onnx_model_path)

# Function to preprocess single image for ONNX model
...
# Collect unseen images and true labels from folder structure
image_paths = []
labels = []

for root, dirs, files in os.walk(unseen_folder):
    for file in files:
        if file.endswith(".jpg"):
            image_paths.append(os.path.join(root, file))
            # Assuming folder name indicates class label: 'Positive' or 'Negative'
            labels.append(1 if "Positive" in root else 0)

# Run inference
preds = []
for img_path in image_paths:
    input_tensor = preprocess_image_onnx(img_path)
    ort_inputs = {ort_session.get_inputs()[0].name: input_tensor}
    ort_outs = ort_session.run(None, ort_inputs)
    pred_class = int(np.round(ort_outs[0][0][0]))  # Binary classification
...
print("Reserved Unseen Evaluation Metrics:")
...
results_df.to_csv(os.path.join(unseen_folder, "reserved_unseen_predictions.csv"), index=False)

print("Evaluation completed and predictions saved ✅")

"""Cell 12 – Farmer Pilot / Field Test"""
...
# Path to folder where farmers upload new images
farmer_data_folder = "/content/drive/MyDrive/farmer_test_images"
onnx_model_path = "/content/drive/MyDrive/Capstone_Project_FAW_Dataset_v4.1/faw_cnn_model.onnx"
...
# Collect all farmer images
farmer_image_paths = [os.path.join(farmer_data_folder, f)
                      for f in os.listdir(farmer_data_folder) if f.endswith(".jpg")]

# Run inference on farmer images
farmer_preds = []
for img_path in farmer_image_paths:
    input_tensor = preprocess_image_onnx(img_path)
    ort_inputs = {ort_session.get_inputs()[0].name: input_tensor}
    ort_outs = ort_session.run(None, ort_inputs)
    pred_class = int(np.round(ort_outs[0][0][0]))  # Binary classification
    farmer_preds.append(pred_class)

# Create a DataFrame to save results
farmer_results_df = pd.DataFrame({
    "image_path": farmer_image_paths,
    "predicted_label": farmer_preds
})

# Save results to CSV
farmer_results_csv = os.path.join(farmer_data_folder, "farmer_test_predictions.csv")
farmer_results_df.to_csv(farmer_results_csv, index=False)

print(f"Farmer Pilot Evaluation Completed ✅")
print(f"Predictions saved to: {farmer_results_csv}")
print(f"Sample results:\n{farmer_results_df.head()}")